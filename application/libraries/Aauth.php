...
class Aauth {
    ...
    public function login($identifier, $pass, $remember = FALSE, $totp_code = NULL) 
    {
        ...
        if ($this->config_vars['ddos_protection'] && !$this->update_login_attempts()) {
            ...
            $this->error(line('aauth_error_login_attempts_exceeded'));
            return FALSE;
        }
        if ($this->config_vars['ddos_protection'] && $this->config_vars['recaptcha_active'] && $this->get_login_attempts() > $this->config_vars['recaptcha_login_attempts']) {
            ...
            if (!$resp->success) {
                $this->error(line('aauth_error_recaptcha_not_correct'));
                return FALSE;
            }
        }
        if ($this->config_vars['login_with_name'] == TRUE) {
            ...
            if (!$identifier OR strlen($pass) < $this->config_vars['min'] OR strlen($pass) > $this->config_vars['max']) {
                $this->error(line('aauth_error_login_failed_name'));
                return FALSE;
            }
            ...
            if (!valid_email($identifier) OR strlen($pass) < $this->config_vars['min'] OR strlen($pass) > $this->config_vars['max']) {
                $this->error(line('aauth_error_login_failed_email'));
                return FALSE;
            }
            $db_identifier = 'email';
        }
        ...
        if ($query->num_rows() > 0) {
            $this->error(line('aauth_error_account_not_verified'));
            return FALSE;
        }
        ...
        if ($query->num_rows() == 0) {
            $this->error(line('aauth_error_no_user'));
            return FALSE;
        }
        if ($this->config_vars['totp_active'] == TRUE AND $this->config_vars['totp_only_on_ip_change'] == FALSE AND $this->config_vars['totp_two_step_login_active'] == FALSE) {
            ...
            if ($query->num_rows() > 0 AND ! $totp_code) {
                $this->error(line('aauth_error_totp_code_required'));
                return FALSE;
            } else {
                if (!empty($totp_secret)) {
                    ...
                    if (!$checkResult) {
                        $this->error(line('aauth_error_totp_code_invalid'));
                        return FALSE;
                    }
                }
            }
        ...
        // if not matches
            ...
            $this->error(line('aauth_error_login_failed_all'));
            return FALSE;
        }
    }
    ...
    public function control($perm_par = FALSE) {
        ...
        if ($this->CI->session->userdata('totp_required')) {
            $this->error(line('aauth_error_totp_verification_required'));
        ...
        if ($perm_par == FALSE) {
            if ($this->is_loggedin()) {
                ...
                $this->error(line('aauth_error_no_access'));
            ...
            if ($this->config_vars['no_permission']) {
                ...
                echo line('aauth_error_no_access');
                die();
            }
        }
    }
    ...
    public function remind_password($email) {
        ...
        if ($query->num_rows() > 0) {
            ...
            $this->CI->email->subject(line('aauth_email_reset_subject'));
            $this->CI->email->message(line('aauth_email_reset_text') . site_url() . $this->config_vars['reset_password_link'] . $ver_code);
            $this->CI->email->send();
    ...
    public function reset_password($ver_code) {
        ...
        if ($query->num_rows() > 0) {
            ...
            $this->CI->email->subject(line('aauth_email_reset_success_subject'));
            $this->CI->email->message(line('aauth_email_reset_success_new_password') . $pass);
            $this->CI->email->send();
    ...
    public function update_user($user_id, $email = FALSE, $pass = FALSE, $username = FALSE) {
        ...
        if ($email != FALSE) {
            if ($this->user_exist_by_email($email)) {
                $this->error(line('aauth_error_update_email_exists'));
                $valid = FALSE;
            }
            $valid_email = (bool) filter_var($email, FILTER_VALIDATE_EMAIL);
            if (!$valid_email) {
        ...
        if ($username != FALSE) {
            if ($this->user_exist_by_username($username)) {
                $this->error(line('aauth_error_update_username_exists'));
                $valid = FALSE;
            }
            if ($username != '' && !ctype_alnum(str_replace($this->config_vars['additional_valid_chars'], '', $username))) {
                $this->error(line('aauth_error_username_invalid'));
                $valid = FALSE;
            }
    ...
    public function get_user($user_id = FALSE) {
        ...
        if ($query->num_rows() <= 0) {
            $this->error(line('Brak wskazanego użytkonika'));
            return FALSE;
        }
        return $query->row();
    }
    ...
    public function send_verification($user_id) {
        ...
        if ($query->num_rows() > 0) {
            ...
            $this->CI->email->from($this->config_vars['email'], $this->config_vars['name']);
            $this->CI->email->to($row->email);
            $this->CI->email->subject(line('Weryfikacja konta'));
            $this->CI->email->message(line('Twój kod weryfikacyjny to:') . $ver_code .
                    line('aauth_email_verification_text') . site_url() . $this->config_vars['verification_link'] . $user_id . '/' . $ver_code);
            $this->CI->email->send();
        }
    }
    ...
    public function create_group($group_name, $definition = '') {
        ...
        $this->info(line('aauth_info_group_exists'));
        return FALSE;
    }
    ...
    public function add_member($user_id, $group_par) {
        ...
        if (!$group_id) {
            ...
            $this->error(line('aauth_error_no_group'));
            return FALSE;
        }
        ...
        $this->info(line('aauth_info_already_member'));
        return TRUE;
    }
    ...
    public function add_subgroup($group_par, $subgroup_par) {
        ...
        if (!$subgroup_id) {
            $this->error(line('aauth_error_no_subgroup'));
            return FALSE;
        }
        ...
        $this->info(line('aauth_info_already_subgroup'));
        return TRUE;
    }
     ...
     */
    public function list_groups($where = null) {

        $query = null;
        if($where) {
            $query = $this->aauth_db->like('name',$where)->get($this->config_vars['groups']);
        }
        else {
            $query = $this->aauth_db->get($this->config_vars['groups']);
        }
    ...
    public function get_group_perms($group_id) {
        ...
        $query = $this->aauth_db->join('aauth_perms', 'aauth_perms.id = aauth_perm_to_group.perm_id');
        $query = $this->aauth_db->where('group_id', $group_id);
        $query = $this->aauth_db->get('aauth_perm_to_group');
        //var_dump($this->aauth_db->last_query());
        if ($query->num_rows() == 0)
            return FALSE;
        //$row = $query->row();
        return $query->result();
    ...
    public function create_perm($perm_name, $definition = '', $module_id = null) {
        ...
        $this->info(line('aauth_info_perm_exists'));
        return FALSE;
    }
    ...
    public function list_perms() {
        $this->aauth_db->join('modules', "modules.module_id = aauth_perms.module_id", 'LEFT');
        $this->aauth_db->order_by('modules.module_id');
        $query = $this->aauth_db->get($this->config_vars['perms']);
        return $query->result();
    }

    public function get_perm($perm_id) {
        $query = $this->aauth_db->join('modules', "modules.module_id = aauth_perms.module_id", 'LEFT');
    ...
    public function send_pm($sender_id, $receiver_id, $title, $message) {
        ...
        if (!is_numeric($receiver_id) OR $sender_id == $receiver_id) {
            $this->error(line('aauth_error_self_pm'));
            return FALSE;
        }
    ...
    public function send_pms($sender_id, $receiver_ids, $title, $message) {
        ...
        foreach ($receiver_ids as $receiver_id) {
            if ($sender_id == $receiver_id) {
                $return_array[$receiver_id] = line('aauth_error_self_pm');
                continue;
            }
            if ($this->is_banned($receiver_id) || !$this->user_exist_by_id($receiver_id)) {
                $return_array[$receiver_id] = line('aauth_error_no_user');
                continue;
            }
    ...
    public function get_pm($pm_id, $user_id = NULL, $set_as_read = TRUE) {
        ...
        if (!is_numeric($user_id) || !is_numeric($pm_id)) {
            $this->error(line('aauth_error_no_pm'));
            return FALSE;
        }
